...
#sns.scatterplot('rate_n_applied_kgha', 'yld_grain_dry_kgha', data=df_data)
...
def buac_to_kgha(buac=200, test_weight=56):
    ...
    lbsha = (ureg.convert(buac, ureg.hectare, ureg.acre) * test_weight)
    kgha = ureg.convert(lbsha, ureg.lbs, ureg.kg)
    return kgha

def kgha_to_buac(kgha=13500, test_weight=56):
    ureg = pint.UnitRegistry()
    kgac = ureg.convert(kgha, ureg.acre, ureg.hectare)
